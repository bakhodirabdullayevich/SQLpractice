...
SELECT DISTINCT COUNT(Category) AS Total_Category FROM Products;
...
SELECT * FROM Sales;
SELECT CustomerID, SUM(SaleAmount) AS TotalSale FROM Sales GROUP BY CustomerID;
...
SELECT DepartmentName, COUNT(EmployeeID) AS TotalEmployee FROM Employees GROUP BY DepartmentName HAVING COUNT(EmployeeID)>5;

--- Medium-Level Tasks (9) ---
-- Write a query to calculate the total sales and average sales for each product category from the Sales table.

SELECT ProductID, SUM(SaleAmount) AS TotalSales, AVG(SaleAmount) AS AverageSales FROM Sales GROUP BY ProductID;

-- Write a query to count the number of employees from the Department HR.

SELECT COUNT(*) AS Number_HR FROM Employees WHERE DepartmentName='HR';

SELECT DepartmentName, COUNT(*) AS TotalEmployees
FROM Employees
GROUP BY DepartmentName HAVING DepartmentName='HR';
...
SELECT DepartmentName, 
	AVG(Salary) AS AverageSalary,
	COUNT(EmployeeID) AS EmployeesNumber
...
SELECT Category, AVG(Price) AS AveragePrice FROM Products GROUP BY Category HAVING AVG(Price) > 400;

-- Write a query that calculates the total sales for each year in the Sales table.

SELECT YEAR(SaleDate), SUM(SaleAmount) AS TotalSale FROM Sales GROUP BY YEAR(SaleDate);

-- Write a query to show the list of customers who placed at least 3 orders.

-- SELECT FirstName + LastName FROM Customers;
-- SELECT CustomerID, Quantity FROM Orders WHERE Quantity >= 3;
...
SELECT DepartmentName, 
	AVG(Salary) AS AverageSalary 
FROM Employees 
GROUP BY DepartmentName HAVING AVG(Salary) > 60000;
...
SELECT Category, AVG(Price) AS AveragePrice FROM Products GROUP BY Category HAVING AVG(Price)>150;
