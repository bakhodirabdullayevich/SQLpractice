...
JOIN 
    ...
    AND e1.EmployeeID < e2.EmployeeID  -- avoids duplicate/reverse pairs
WHERE 
...
WHERE Salary > (SELECT 
	-- e.Name AS EmployeeName, 
	AVG(e.Salary) AS AveregeDepSalary
FROM 
	Employees e 
GROUP BY 
	e.DepartmentID) 
...
LEFT JOIN 
	Payments p ON o.OrderID = p.OrderID
WHERE 
	o.OrderID IS NULL AND YEAR(o.OrderDate) < 2020;
...
WHERE 
	e.ManagerID IS NOT NULL AND e.Salary > 60000;
...
JOIN 
    Employees e2 ON e1.ManagerID = e2.ManagerID
                AND e1.EmployeeID < e2.EmployeeID  -- avoid duplicate pairs
WHERE 
    e1.ManagerID IS NOT NULL
    AND e1.Salary > 60000
    AND e2.Salary > 60000;
