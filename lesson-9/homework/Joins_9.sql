/*
Joins (Only inner with table relationships)
...
SELECT e.Name FROM Employees e JOIN Departments d ON e.DepartmentID = d.DepartmentID;

-- Using Students, Enrollments table List student names and their enrolled course IDs.

SELECT s.Name AS StudentName, e.CourseID FROM Students s JOIN Enrollments e ON s.StudentID = e.StudentID;
...
SELECT * FROM Orders o JOIN Products p ON o.ProductID = p.ProductID WHERE o.Quantity > p.StockQuantity;
...
FROM 
	Products p 
JOIN 
	Categories c ON c.CategoryID = p.Category 
WHERE 
...
SELECT * FROM Orders o JOIN Customers c ON o.CustomerID = c.CustomerID WHERE c.Country = 'Germany' AND o.TotalAmount > 100;


--- Hard --- (Do some research for the tasks below)
-- Using Employees table List all pairs of employees from different departments.
--- ???
SELECT * FROM Employees e1 JOIN Employees e2 ON e1.EmployeeID = e2.EmployeeID WHERE e1.DepartmentID <> e2.DepartmentID
...
WHERE
	s.StudentID NOT IN (SELECT s.StudentID FROM Students s);
-----------------------------------------------------------------
SELECT 
    s.StudentID,
    s.Name AS StudentName
...
WHERE 
	e1.EmployeeID <> e2.ManagerID AND e1.Salary >= e2.Salary;
-------------------------------------------------------------
SELECT 
